...
class DatasetManager:
    ...
    def __init__(self, dataset_name):
        ...
        self.resource_multitasking_col = dataset_confs.resource_multitasking_col[self.dataset_name]
        self.totaltimeofcase_col = dataset_confs.totaltimeofcase_col[self.dataset_name]
    ...
    def read_dataset(self):
        ...
        data[self.timestamp_col] = pd.to_datetime(data[self.timestamp_col], format='mixed')
    ...
    def get_pos_case_length_quantile(self, data, quantile=0.90):  # my extension
        temp = data[self.label_col].unique()
        temp=np.sort(temp)
        list0 = []
        for i in temp:
            temp0 = data[data[self.label_col] == i].groupby(self.case_id_col).size()
            temp1 = temp0.quantile(quantile)
            list0.append(temp1)
        result = np.mean(list0)
    ...
    def get_label_time(self,data):
        return data.groupby(self.case_id_col).first()[self.totaltimeofcase_col]
    ...
    def get_label_numeric(self, data):  # my extension #number_of_bunch
        y = (self.get_label(data).values)  # one row per case
        y = list(map(int, y.tolist()))

        return y

    def get_label_numeric_reg(self, data):  # my extension #number_of_bunch
        y = (self.get_label(data).values)  # one row per case
        y = list(map(float, y.tolist()))
        return y



    def get_class_ratio(self, data):  # my extension
        class_freqs = data[self.label_col].value_counts()
        class_freqs = class_freqs.apply(lambda x: x / class_freqs.sum())
        result = class_freqs.mean()
        return result

    def get_class_ratio_2(self, data):  # my extension
        class_freqs = data[self.label_col].value_counts()
        max_val = int(class_freqs.idxmax())
        return max_val
    ...
    def get_stratified_split_generator_reg(self, data, n_splits=5, shuffle=True, random_state=22):
        grouped_firsts = data.groupby(self.case_id_col, as_index=False).first()
