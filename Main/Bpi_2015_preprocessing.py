...
"** This file serves as the preprocessing phase for preparing the BPIC 2015 datasets.**"
input_data_folder = main.input_data_folder
output_data_folder = main.output_data_folder

"Based on the selected dataset in the main file, change the range for BPIC15_i from (i, i+1)."
filenames = ["BPIC15_%s.csv" % (municipality) for municipality in range(1, 2)]
...
static_cat_cols = ["case:Responsible_actor"]
static_num_cols = ['case:SUMleges']
dynamic_num_cols = []
...
def split_parts(group, parts_col="case:parts"):
...
for filename in filenames:
    ...
    data = data[data["case:caseStatus"] == "G"]  # G is closed, O is open
    ...
    ####### select equal percent per label
    data = data2
    del (data2)
    ...
    if main.inter_intra_mode == 'inter+intra':
        ...
        # resource multitasking calculation
    ...
    if main.inter_intra_mode == 'inter+intra':
        for col in static_cols + dynamic_cols + ["open_cases",
                                                 "resource_multitasking"] + other_resource_experience:  # , totaltimeofcase_col
            dt_labeled[col] = grouped[col].transform(lambda grp: grp.fillna(method='ffill'))
    elif main.inter_intra_mode == 'intra':
    ...
    all_columns = main.static_cat_cols + main.dynamic_cat_cols + main.dynamic_num_cols + main.static_num_cols_regression + [
        'time:timestamp', case_id_col, label_col]

    dt_labeled = dt_labeled.loc[:, all_columns]

    dt_labeled.to_csv(os.path.join(output_data_folder, "%s_%s.csv" % (filename[:-4], relevant_offer_events[0])),
                      sep=";",
                      index=False)
